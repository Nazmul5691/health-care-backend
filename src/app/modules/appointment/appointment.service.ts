...
const createAppointment = async (user: IAuthUser, payload: any) => {
    ...
    const result = await prisma.$transaction(async (tnx) => {
        ...
        const session = await stripe.checkout.sessions.create({
            ...
            success_url: `${process.env.FRONTEND_URL || 'http://localhost:3000'}/payment/success?appointmentId=${appointmentData.id}`,
            cancel_url: `${process.env.FRONTEND_URL || 'http://localhost:3000'}/dashboard/my-appointments`,
        });
...
const initiatePaymentForAppointment = async (appointmentId: string, user: IAuthUser) => {
    ...
    const session = await stripe.checkout.sessions.create({
        ...
        success_url: `${process.env.FRONTEND_URL || 'http://localhost:3000'}/payment/success?appointmentId=${appointment.id}`,
        cancel_url: `${process.env.FRONTEND_URL || 'http://localhost:3000'}/dashboard/my-appointments`,
    });
